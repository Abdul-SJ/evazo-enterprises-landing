...
export default function FeaturedProduct() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // Text content reveal (fade-up stagger)
      gsap.fromTo(
  ...
  const handleWhatsAppClick = (e: React.MouseEvent) => {
    e.preventDefault();
    window.open(
      "https://wa.me/923374259849?text=Hi%20Evoza%20Enterprises,%20I%20would%20like%20to%20discuss%20custom%20hoodies%20and%20fits%20for%20my%20brand.",
      "_blank"
    );
  };
  ...
  return (
    <section
      id="featured-product"
      ref={containerRef}
      className="py-24 md:py-36 max-w-7xl mx-auto px-6 md:px-12"
    >
      <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-16">
        {/* Left Side: Premium Image Container (Increased Size) */}
        <div className="w-full lg:w-1/2 relative flex justify-center">
          <div className="absolute inset-0 bg-brand/5 rounded-3xl transform -translate-x-4 translate-y-4 -z-10" />
          <div
            ref={imageContainerRef}
            className="relative w-full aspect-[3/4] max-w-lg rounded-2xl overflow-hidden shadow-2xl border border-border-primary/60 group"
