...
export default function ManufacturingProcess() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      // Timeline cards staggered entry animation (Delay: 0.12s between cards)
      gsap.fromTo(
      ...
      // Line progress indicator animation (scrubbed with scroll)
      if (lineRef.current) {
        gsap.fromTo(
          lineRef.current,
          { scaleX: 0 },
  ...
  const handleWhatsAppClick = (e: React.MouseEvent) => {
    e.preventDefault();
    window.open(
      "https://wa.me/923374259849?text=Hi%20Evoza%20Enterprises,%20I%20would%20like%20to%20know%20more%20about%20your%20manufacturing%20process%20and%20timelines.",
      "_blank"
    );
  };

  return (
    <section ref={containerRef} className="py-24 md:py-36 bg-white relative overflow-hidden border-t border-border-primary/50">
            ...
            {steps.map((step, idx) => {
              ...
              return (
                <div key={idx} className="flex flex-col items-center text-center group">
                  {/* Step icon circle */}
                  <div className="h-22 w-22 bg-bg-secondary border-2 border-border-primary group-hover:border-brand rounded-full flex items-center justify-center mb-6 text-text-muted group-hover:text-brand transition-all duration-300 shadow-sm group-hover:shadow-md relative bg-white">
